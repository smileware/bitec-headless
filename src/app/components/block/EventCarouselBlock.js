...
import { useRecentEvents } from '../../hooks/useBlockQueries';
...
export default function EventCarouselBlock(props) {
  const { data, isLoading } = useRecentEvents(9);
  const events = data ?? [];
  ...
  // Only skeleton when there is truly no cached/prefetched data yet
  if (isLoading && data === undefined) {
    return <CarouselSkeleton {...props} />;
  }
  ...
  // SSR + first client paint: static grid (matches). Swiper only after mount.
  if (!mounted) {
    return (
      <div {...props} className="swiper-event-carousel">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-[20px] lg:px-0">
