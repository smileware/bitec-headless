...
// Counts how many instances have initialised per pathname.
// Using modulo when selecting halls handles repeated navigations safely.
...
export default function BitecLiveHallCarouselBlock(props) {
    ...
    // Assign a stable, unique position index to this component instance.
    // useRef guarantees the same object identity across re-renders, so the
    // assignment inside the if-block only ever runs once per mount — even when
    // all block instances share the same props.id value.
    const myIndexRef = useRef(null);
    if (myIndexRef.current === null) {
        const count = pageCounters.get(pathname) ?? 0;
        myIndexRef.current = count;
        pageCounters.set(pathname, count + 1);
    }
    ...
    // Pick the hall that belongs to this instance's position in the page.
    // Modulo keeps selection correct even if the counter accumulated across
    // client-side navigations (e.g. 3, 4, 5 → 3%3=0, 4%3=1, 5%3=2).
    const hallData = useMemo(() => {
        if (!bitecLiveHallData?.bitecLiveHalls?.length) return null;
        const halls = bitecLiveHallData.bitecLiveHalls;
        const safeIndex = myIndexRef.current % halls.length;
        return halls[safeIndex] ?? halls[0];
    }, [bitecLiveHallData]);
    ...
    const paginationEl = `.swiper-pagination-${cardId}`;
    const nextEl = `.swiper-button-next-${cardId}`;
    const prevEl = `.swiper-button-prev-${cardId}`;
            ...
            <div className="gs-swiper-init">
                <div className="bitec-live-hall-card bg-white overflow-hidden mb-8">
                    <div className="flex flex-col lg:flex-row">
                        ...
                        <div className="w-full lg:w-[70%] relative">
                            {hallData.gallery && hallData.gallery.length > 0 && (
                                <Swiper
                                    modules={[Navigation, Pagination]}
                                    slidesPerView={1}
                                    spaceBetween={0}
                                    speed={400}
                                    loop={hallData.gallery.length > 1}
                                    autoHeight={false}
                                    grabCursor={true}
                                    freeMode={false}
                                    centeredSlides={false}
                                    autoplay={hallData.gallery.length > 1 ? {
                                        delay: 5000,
                                ...
                                >
                                    {hallData.gallery.map((image, imageIndex) => (
                                    ...
                                    <div className={`swiper-pagination swiper-pagination-${cardId}`}></div>
                                </Swiper>
                            )}
                            <div className={`swiper-button-prev _mobile swiper-button-prev-${cardId}`}>
                            ...
                            <div className={`swiper-button-next _mobile swiper-button-next-${cardId}`}>
                                <svg width="14" height="15" viewBox="0 0 14 15" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
